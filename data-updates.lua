...
lib.moveeffect('slag-processing-2', 'slag-processing-1', 'slag-processing-2', slag2start + 1)
...
data.raw.technology['electronics'].prerequisites = { 'automation', 'slag-processing-1', 'angels-coal-processing' }
...
if not data.raw.technology['sct-research-t2'] then
  data.raw.recipe['logistic-science-pack'].enabled = false
  table.insert(data.raw.technology['automation-2'].effects, {type = 'unlock-recipe', recipe = 'logistic-science-pack'})
end
...
if data.raw.fluid['hydrazine'] then
  ...
  if hydrazinevalue:sub(-2) == 'kJ' then
    ...
    lib.substingredient('solid-fuel-hydrazine', 'gas-hydrazine', nil, 24000 / hydrazinevaluekj)
  end
end
